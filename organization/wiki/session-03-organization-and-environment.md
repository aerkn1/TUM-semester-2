...
Wiki note: `organization/wiki/session-03-organization-and-environment.md`
...
```mermaid
flowchart TD
    Env[Organization and environment] --> Boundary[Boundary problem]
    Boundary --> TaskEnv[Task environment]
    Boundary --> GlobalEnv[Global environment]

    TaskEnv --> Suppliers[Suppliers]
    TaskEnv --> Buyers[Buyers]
    TaskEnv --> Competitors[Competitors]
    TaskEnv --> Collaborators[Collaborators]

    GlobalEnv --> Tech[Technological]
    GlobalEnv --> Legal[Political-legal]
    GlobalEnv --> Socio[Socio-cultural]
    GlobalEnv --> Eco[Ecological]
    GlobalEnv --> Macro[Macroeconomic]

    Env --> Interdependence[Interdependence]
    Interdependence --> Pooled[Pooled]
    Interdependence --> Sequential[Sequential]
    Interdependence --> Reciprocal[Reciprocal]

    Env --> Uncertainty[Environmental uncertainty]
    Uncertainty --> Change[Environmental change]
    Uncertainty --> Complexity[Environmental complexity]

    Env --> Theories[Theories of interaction]
    Theories --> Contingency[Contingency theory]
    Contingency --> Fit[Structure-environment fit]
    Fit --> Mechanistic[Mechanistic]
    Fit --> Organic[Organic]
    Theories --> RDT[Resource dependence]
    RDT --> Need[Need]
    RDT --> Scarcity[Scarcity]
    RDT --> Substitutes[Substitutes]
    Theories --> Ecology[Population ecology]
    Ecology --> Variation[Variation]
    Ecology --> Selection[Selection]
    Ecology --> Retention[Retention]

    Env --> Responses[Managing environment]
    Responses --> BoundarySpanning[Boundary spanning]
    Responses --> PR[Public relations]
    Responses --> Differentiation[Differentiation]
    Responses --> Sensing[Sensing]

    Env --> BP[BP Part 2]
    BP --> Dependencies[Interorganizational dependencies]
    BP --> InfoAsymmetry[Information asymmetry]
    BP --> UncertaintyAbsorption[Failed uncertainty absorption]
```
