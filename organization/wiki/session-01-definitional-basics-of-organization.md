...
Wiki note: `organization/wiki/session-01-definitional-basics-of-organization.md`
...
```mermaid
flowchart TD
    Org[Organization] --> Social[Social entity]
    Org --> Goals[Goal-directed]
    Org --> Structure[Deliberately structured]
    Org --> Env[Environment-linked]

    Org --> Problems[Key organizing problems]
    Problems --> Tasks[Structuring tasks]
    Problems --> Motivation[Motivation beyond compliance]
    Problems --> Uncertainty[Coping with uncertainty]
    Problems --> Emergence[Emergent processes]
    Problems --> Change[Change and adaptability]

    Org --> Views[Three views]
    Views --> Rational[Rational system]
    Views --> Natural[Natural system]
    Views --> Open[Open system]

    Structure --> Formal[Formal structure]
    Formal --> Roles[Roles]
    Formal --> Reporting[Reporting lines]
    Formal --> Rules[Rules/procedures]
    Structure --> Practice[Organizing in practice]
    Practice --> Workarounds[Workarounds]
    Practice --> Informal[Informal communication]
    Practice --> Local[Local problem solving]

    Org --> Management[Management]
    Management --> Actors[Managers as actors]
    Management --> Activity[Management as activity]
```
