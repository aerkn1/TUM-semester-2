...
Wiki note: `organization/wiki/session-02-formal-organizational-design.md`
...
```mermaid
flowchart TD
    FormalDesign[Formal organizational design] --> Weber[Weber bureaucracy]
    Weber --> Rules[Rules and procedures]
    Weber --> Roles[Roles]
    Weber --> Incentives[Incentives]
    Weber --> Structure[Structure]

    Rules --> Control[Control and reduced discretion]
    Rules --> Artifact[Artifacts do not determine action]
    Artifact --> Interpretation[Interpretation/adaptation/workarounds]

    Roles --> Monitoring[Monitoring]
    Roles --> Substitution[Substitution]
    Roles --> Common[Common perspective]

    Incentives --> Behavior[Encourage behavior]
    Incentives --> Attention[Shape attention]
    Incentives --> Unintended[Unintended consequences]
    ...
    Functional --> Efficiency[Efficiency/control]
    Horizontal --> Flexibility[Coordination/learning/flexibility]
    Matrix --> Dual[Dual demands]

    FormalDesign --> BP[BP case]
    BP --> Hierarchy[Decision rights]
    BP --> Procedures[Management of change]
    BP --> Reporting[Onshore/offshore reporting]
    BP --> Diagnostics[Cement bond log choice]
    BP --> Accountability[Budget/schedule visibility]
```
