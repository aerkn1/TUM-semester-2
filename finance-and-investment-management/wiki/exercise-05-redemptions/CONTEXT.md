...
Source note: `exercise-05-redemptions.md`
...
| Term | Definition | Aliases to avoid |
|---|---|---|
| **Redemption** | Repayment of loan principal over time. | interest payment |
| **Installment Repayment** | A repayment structure with equal principal repayments plus declining interest. | annuity repayment |
| **Annuity Repayment** | A repayment structure with constant total payments split between interest and principal. | installment repayment |
| **Grace Period** | A period where principal repayment is deferred, with interest either paid or capitalized. | free period |
| **Outstanding Principal** | The remaining loan amount on which interest is calculated. | payment amount |
| **Amortization Schedule** | A table showing interest, principal repayment, total payment, and remaining balance by period. | cash-flow list |
