...
### Prompt 6: Sequencing CAPM, Debt Financing, WACC, And Redemptions

User question:

> I wanna add extra clarification to the cost of capital topic for finance: so the euqity cost is defined via CAPM by calculating the market risk for the project and debt cost of capital is also evaluated based on the required capital inclusion via the debt financin plan based on required amount and possible options and then it is scheduled via redemption/annuity. Eventually the WACC is generated by blending the euqity and debt based return rate expectations plus the projects own operational risks?

Professor feedback:

- The direction is good: equity cost is commonly estimated through CAPM, using project-relevant systematic market risk.
- Debt cost comes from the required return lenders demand for the chosen debt risk, amount, maturity, seniority, and available financing alternatives.
- Redemptions and annuities schedule contractual debt service after debt terms are chosen; they do not define the debt cost inside WACC.
- WACC blends equity cost and after-tax debt cost using target market-value weights.
- Correction: do not add "project operational risk" as a third WACC component by default. Project operating risk should already be captured in the project beta/risk class, equity cost, debt risk, and target capital structure.

Refined mental model:

```text
Project operating risk
-> project beta / comparable risk class
-> CAPM estimate of r_E
-> debt financing plan and market debt cost estimate for r_D
-> WACC from r_E, after-tax r_D, and target weights
-> project NPV
-> redemption / annuity schedule for debt-service feasibility
```

Quality label: `green/yellow`, because the sequence is right but the "plus operational risk" wording risks double-counting.
...
Project operating risk
= embedded in beta, r_E, r_D, and target weights; not added again unless instructed.
```
...
| Operational risk in WACC | `yellow` | Operating risk should be embedded in project beta/risk class and required returns; do not add it as a separate third WACC component unless instructed. | Explain how project operating risk enters WACC without double-counting. |
...
6. Build the full sequence from project risk -> CAPM `r_E` -> debt plan `r_D` -> WACC -> NPV -> redemption schedule.
