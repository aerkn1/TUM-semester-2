# Ubiquitous Language: Topic 02: Forecasting

Source note: `topic-02-forecasting.md`
...
| Term | Definition | Aliases to avoid |
|---|---|---|
| **Forecast** | An estimate of future demand or another uncertain variable used before the actual value is known. | target, plan |
| **Point Estimate** | A single-number forecast for a future value. | confidence interval |
| **Forecast Error** | The realized difference between actual value and forecast, often e_t = A_t - F_t. | mistake only |
| **Mean Error** | Average signed forecast error used to detect bias. | MAD |
| **MAD** | Mean absolute deviation, the average absolute forecast error. | MSE |
| **MSE** | Mean squared error, the average squared forecast error that penalizes large misses strongly. | MAD |
| **Control Limit** | A threshold used to judge whether forecast errors are unusually large or systematic. | target service level |
| **Exponential Smoothing** | A forecasting method that updates the forecast with a weighted average of the newest actual value and previous forecast. | moving average |
