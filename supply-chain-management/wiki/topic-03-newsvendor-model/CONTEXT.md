# Ubiquitous Language: Topic 03: Newsvendor Model

Source note: `topic-03-newsvendor-model.md`
...
| Term | Definition | Aliases to avoid |
|---|---|---|
| **Newsvendor Model** | A single-period inventory model that chooses order quantity before uncertain demand is realized. | forecasting model |
| **Underage Cost** | The marginal loss from ordering one unit too few. | selling price |
| **Overage Cost** | The marginal loss from ordering one unit too many. | purchase cost always |
| **Critical Fractile** | The cost-based target service level c_u/(c_u+c_o). | probability of demand |
| **Demand Quantile** | The demand value whose cumulative probability equals or first exceeds a target service level. | average demand |
| **Salvage Value** | The value recovered from unsold inventory after the selling period. | leftover cost |
| **Discrete CDF Rule** | For discrete demand, choose the smallest Q with F(Q) >= SL. | closest CDF rule |
