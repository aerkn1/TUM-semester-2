...
For Supply Chain Management clarification sessions, prioritize the bridge between formulas and operational decisions. Explain each metric, model, or formula by linking `operational decision -> tradeoff -> formula inputs -> managerial interpretation -> failure mode`. Use concrete production, retail, fulfillment, procurement, inventory, and service-level analogies. When the user later asks to update the files, add these decision-use explanations, analogies, formula-intuition notes, and exam-trap corrections to the topic note and `CONTEXT.md`.
