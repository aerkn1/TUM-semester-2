...
```mermaid
flowchart TD
    Need[Customer value and willingness to pay] --> Behavior[Behavioral price evaluation]
    Behavior --> Reference[Reference price and fairness]
    Behavior --> Choice[Compromise and decoy effects]
    Need --> Differentiate[Price differentiation]
    Differentiate --> Forms[Time, place, demographic, benefit, quantity]
    Differentiate --> Dynamic[Dynamic pricing and yield management]
    Differentiate --> Bundle[Bundling and two-part tariffs]
    Need --> Determine[Price determination]
    Determine --> Cost[Full and marginal cost pricing]
    Determine --> Market[Break-even and contribution margin]
    Determine --> Risk[Expected value under uncertainty]
    Determine --> Marginal[Elasticity and MR equals MC]
    Cost --> Decision[Price decision]
    Market --> Decision
    Risk --> Decision
    Marginal --> Decision
    Decision --> Check[Demand, competition, fairness, profit]
```
