...
Wiki note: `marketing/wiki/chapter-01-basic-concepts-of-marketing.md`
...
```mermaid
flowchart TD
    Marketing[Marketing] --> Exchange[Value exchange]
    Exchange --> Needs[Customer needs]
    Exchange --> Supplier[Supplier offering]
    Marketing --> Triangle[Marketing triangle]
    Triangle --> CustomerNeeds[Customer needs/wishes]
    Triangle --> OwnOffer[Own service offering]
    Triangle --> Competitors[Competitors' offerings]
    Triangle --> Advantage[Sustainable competitive advantage]

    Marketing --> Process[Management process]
    Process --> Analysis[Situation analysis and forecast]
    Analysis --> Goals[Marketing goals]
    Goals --> Strategy[Marketing strategy]
    Strategy --> Mix[4Ps marketing mix]
    Mix --> Product[Product]
    Mix --> Price[Price]
    Mix --> Promotion[Promotion]
    Mix --> Place[Place]
    Mix --> Implementation[Implementation]
    Implementation --> Controlling[Marketing controlling]

    Marketing --> SuccessChain[Success chain]
    SuccessChain --> Value[Customer value]
    Value --> Acquisition[Customer acquisition]
    ...
    Satisfaction --> Disconfirmation[Expectation vs performance]
    Disconfirmation --> Complaints[Complaints]
    Complaints --> Recovery[Recovery]
    Recovery --> ThreeRs[3Rs: recruitment, retention, recovery]

    Marketing --> ConsumerBehavior[Consumer behavior]
    ConsumerBehavior --> Segmentation[Segmentation]
    Segmentation --> Targeting[Targeting]
    ConsumerBehavior --> Goods[Types of goods]
    Goods --> Search[Search properties]
    Goods --> Experience[Experience properties]
    Goods --> Trust[Trust properties]
    ConsumerBehavior --> Involvement[Involvement]
    Involvement --> Habitual[Habitual buying]
    Involvement --> Impulsive[Impulsive buying]
```
