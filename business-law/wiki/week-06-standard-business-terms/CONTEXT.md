...
```mermaid
flowchart TD
    Clause[Specific clause] --> SBT{SBT under Section 305 I?}
    SBT -->|No| Ordinary[Ordinary contract term]
    SBT -->|Yes| Negotiated{Negotiated in detail?}
    Negotiated -->|Yes| Individual[Individual agreement]
    Negotiated -->|No| Incorporated{Incorporated?}
    Incorporated -->|No| S306[Section 306 gap filling]
    Incorporated -->|Yes| Ambiguous{Ambiguous?}
    Ambiguous -->|Yes| Contra[Interpret against user]
    Ambiguous -->|No| Control[Content control]
    Contra --> Control
    Control --> Valid{Valid?}
    Valid -->|Yes| Applies[Clause applies]
    Valid -->|No| S306
```
