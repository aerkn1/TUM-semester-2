...
```mermaid
flowchart TD
    Exam[Mock exam fact pattern] --> QType{Question type?}
    QType -->|Case 1 claim| Case1[Hardware provision]
    QType -->|Case 2 validity| Case2[Tablet purchase]
    QType -->|Theory| Theory[Short answer]

    Case1 --> Claim[Claim basis]
    Claim --> SRep[S-GmbH represented by D2]
    Claim --> RRep[R-oHG represented by P]
    SRep --> GmbH35[Section 35 GmbHG]
    RRep --> Prokura[Sections 49-50 HGB]
    Prokura --> Internal[Internal B2C limit]
    Internal --> Bound[Likely externally bound]

    Case2 --> Formation[Formation]
    Formation --> Display[Display is invitation]
    Formation --> OfferAccept[Offer + acceptance]
    Case2 --> Deceit[Section 123 deceit]
    Deceit --> Void[Section 142 ex tunc]

    Theory --> Form[GbR/oHG/GmbH choice]
    Theory --> Damages[Section 280]
    Theory --> Process[Legislative process]
    Theory --> Impossible[Section 275]
