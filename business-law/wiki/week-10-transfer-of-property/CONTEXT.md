...
```mermaid
flowchart TD
    Facts[Asset transfer facts] --> Object{What is transferred?}
    Object -->|Movable thing| Movable[Section 929 route]
    Object -->|Land| Land[Sections 873, 925 route]
    Object -->|Claim/right| Claim[Section 398 assignment / Section 453 sale]
    Movable --> Agreement[Transfer agreement]
    Movable --> Delivery[Delivery or replacement]
    Movable --> Persist[Agreement at delivery]
    Movable --> Auth{Authorization?}
    Auth -->|Owner/authorized| OwnerTransfer[Ownership passes]
    Auth -->|No| GoodFaith{Good-faith acquisition?}
    GoodFaith --> S932[Section 932 good faith]
    S932 --> S935{Lost/stolen/involuntary loss?}
    S935 -->|Yes| NoAcquire[No good-faith acquisition]
    S935 -->|No| Acquire[Acquirer becomes owner]
    Land --> Conveyance[Conveyance]
    Conveyance --> Register[Land-register entry]
    Claim --> Assignment[Assignor + assignee agreement]
```
